...
class plotter_after_tpc():
    def __init__(self, data_folder, run_number, angle):
        ...
        for p in range(0, 4):
            cl_pd_list.append(
                pd.read_feather(f"{data_folder}/perf_out/{self.run}/match_cl_{p}_TPC-zstd.feather"))
            self.track_pd_list.append(
                pd.read_pickle(f"{data_folder}/perf_out/{self.run}/tracks_pd_{p}_TPC.gzip",
                               compression="gzip"))
            eff_pd_l.append(
                pd.read_feather(f"{data_folder}/perf_out/{self.run}/eff_pd_{p}_TPC-zstd.feather"))
