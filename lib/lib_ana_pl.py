...
raw_root_folder = "/media/disk2T/VM_work_zone/data/raw_root/"
perf_out_folder = "/media/disk2T/VM_work_zone/data/perf_out/"
...
def get_run_data(runs, dtype="h", planar=0):
    folder = raw_root_folder
    if dtype == "h":
        filename = "hit_data"
    if dtype == "t":
    ...
    if dtype == "1D_eff":
        filename = f"match_cl_{planar}.gzip"
        folder = perf_out_folder
    if dtype == "eff":
        filename = f"eff_pd_{planar}.gzip"
        folder = perf_out_folder
    data_list = []
    for run in runs:
        data_list.append(pd.read_pickle(f"{folder}/{run}/{filename}.pickle.gzip", compression="gzip"))
...
def write_html(fig, name, width=width_d, height=height_d):
    ...
    fig.write_html(f"/media/disk2T/VM_work_zone/CIVETTA/workspaces/alberto/plot_out/{name}.html",
...
def charge_vs_time_plot_log(data_pd_cut_2, view="e", log=True):
    ...
    hist_2d = np.histogram2d(data_pd_cut_2.charge_SH, -data_pd_cut_2.l1ts_min_tcoarse * 6.25 * 10 ** -3,
                             bins=[200, 267], range=[[0, 60], [-1567 * 6.25 * 10 ** -3, -1300 * 6.25 * 10 ** -3]])
...
def charge_vs_strip_log(data_pd_cut_2, view="e", log=True):
    if view != "e":
        data_pd_cut_2 = data_pd_cut_2[data_pd_cut_2[f"strip_{view}"] >= 0]
    hist_2d = np.histogram2d(data_pd_cut_2.charge_SH, data_pd_cut_2[f"strip_{view}"], bins=[200, 128],
                             range=[[0, 60], [0, 128]])
    x_bin = ((hist_2d[1][1:] + hist_2d[1][:-1]) / 2)
    y_bin = ((hist_2d[2][1:] + hist_2d[2][:-1]) / 2)
    if log:
...
def charge_vs_time_plot_log_multi(data_pd_cut_2, view="e", log=True, titles=["False"]):
    ...
    for n, run in enumerate(data_gr.groups):
        ...
        hist_2d = np.histogram2d(run_pd.charge_SH, -run_pd.l1ts_min_tcoarse * 6.25 * 10 ** -3, bins=[2, 70],
                                 range=[[0, 60], [-1430 * 6.25 * 10 ** -3, -1361 * 6.25 * 10 ** -3]])
        x_bin = ((hist_2d[1][1:] + hist_2d[1][:-1]) / 2)
        y_bin = ((hist_2d[2][1:] + hist_2d[2][:-1]) / 2)
        if log:
            z_value = np.log10(hist_2d[0])
        ...
        fig.append_trace(go.Heatmap(x=y_bin, y=x_bin, z=z_value, colorscale="viridis", coloraxis="coloraxis", name=run),
                         row=1, col=n + 1)
    fig.update_layout(template=standard_template)
...
def charge_vs_time_plot_log_multi(data_pd_cut_2, view="e", log=True, titles=["False"], run_list=["False"]):
    ...
    for n, run in enumerate(run_list):
        run_pd = data_gr.get_group(run)
        hist_2d = np.histogram2d(run_pd.charge_SH, -run_pd.l1ts_min_tcoarse * 6.25 * 10 ** -3, bins=[800, 70],
                                 range=[[0, 60], [-1430 * 6.25 * 10 ** -3, -1361 * 6.25 * 10 ** -3]])
        x_bin = ((hist_2d[1][1:] + hist_2d[1][:-1]) / 2)
        y_bin = ((hist_2d[2][1:] + hist_2d[2][:-1]) / 2)
        if log:
...
def charge_vs_time_plot_log_multi_angle(data_pd_cut_2, view="e", log=True, titles=["False"], runs_order=["False"]):
    ...
    for n, run in enumerate(runs_order):
        ...
        hist_2d = np.histogram2d(run_pd.charge_SH, -run_pd.l1ts_min_tcoarse * 6.25 * 10 ** -3, bins=[200, 70],
                                 range=[[0, 60], [-1430 * 6.25 * 10 ** -3, -1361 * 6.25 * 10 ** -3]])
        x_bin = ((hist_2d[1][1:] + hist_2d[1][:-1]) / 2)
        y_bin = ((hist_2d[2][1:] + hist_2d[2][:-1]) / 2)
        if log:
...
def charge_vs_time_plot_log_multi_drift(data_pd_cut_2, view="e", log=True, titles=["False"]):
    ...
    fig = make_subplots(rows=2, cols=data_gr.ngroups, subplot_titles=subplot_titles + subplot_titles,
    ...
    for n, run in enumerate(data_gr.groups):
        ...
        hist_1d = np.histogram(-run_pd_c.l1ts_min_tcoarse * 6.25 * 10 ** -3, bins=70, density=True,
                               range=[-1430 * 6.25 * 10 ** -3, -1361 * 6.25 * 10 ** -3])
...
def signal_ratio_plot(data_pd_cut_2, sel_run, planar):
    ...
    sin_cut = data_pd_cut_2[((data_pd_cut_2.l1ts_min_tcoarse < signal_upper_limit) & (
                data_pd_cut_2.l1ts_min_tcoarse > signal_lower_limit) & (data_pd_cut_2.planar == planar) & (
                                         data_pd_cut_2.charge_SH > 5))]
    nois_cut = data_pd_cut_2[((data_pd_cut_2.l1ts_min_tcoarse > signal_upper_limit) | (
                data_pd_cut_2.l1ts_min_tcoarse < signal_lower_limit) & (data_pd_cut_2.planar == planar))]
...
def noise_hits_plot(data_pd_cut_2, sel_run, planar):
    ...
    noise_pd = noise_pd.groupby(noise_pd.subRunNo.divide(5).round().multiply(5)).agg(
        {"subRunNo": "mean", "time_r": "mean", "noise_rate": "mean"})
...
def signal_heatmap_plot(cluster_pd_2D_pre_2, sel_binning):
    fig = px.density_heatmap(x=cluster_pd_2D_pre_2.cl_pos_x * 0.0650, y=cluster_pd_2D_pre_2.cl_pos_y * 0.0650,
                             marginal_x="histogram", marginal_y="histogram", nbinsx=int(128 / sel_binning),
                             nbinsy=int(128 / sel_binning), color_continuous_scale=px.colors.sequential.Viridis)
    fig.update_layout(
...
def clusters_vs_tim_plot(cluster_pd_2D_pre_2, sel_run):
    ...
    cluster_pd_2D_pre_2["time_r"] = cluster_pd_2D_pre_2.apply(lambda row: time[(int(row.run))][str(int(row.subrun))],
                                                              axis=1)
...
def distr_charge_plot_2D(cluster_pd_2D_pre_2):
    ...
    fig = go.Figure()
    fig.add_trace(go.Histogram(x=cluster_pd_2D_cut.cl_charge, opacity=0.75, xbins=dict(size=range_b / nbins, start=0),
                               name="Charge histogram"))
    fig.update_layout(
        yaxis_title="Count",
...
def distr_pos_plot_1D(cl_pd, view):
    ...
    for run in cl_pd_g.groups:
        cl_pd = cl_pd_g.get_group(run)
        data_dict[run] = np.histogram(cl_pd[f"cl_pos_{view}"] * 0.0650, bins=128, range=[-0.001, 128 * 0.0650],
                                      density=False)
