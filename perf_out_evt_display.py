...
class event_visualizer:
    ...
    """
    def __init__(self, cluster_pd_1D,cluster_pd_match, tracks_pd, hit_pd, put, correction):
    ...
    def plot_evt(self, event):
        ...
        data_pd_evt["cluster_eff"] = data_pd_evt["hit_id"].isin(hit_ids)
        color_discrete_map = {False: 'cyan', True: 'red'}
        data_pd_evt["time"] = (1569 - data_pd_evt["l1ts_min_tcoarse"])
        ...
        data_pd_evt = self.data_pd[(self.data_pd["count"] == event) & (self.data_pd["planar"] == self.put) & (self.data_pd["strip_y"] > 0) & (
                    self.data_pd["l1ts_min_tcoarse"] > 1370) & (self.data_pd["l1ts_min_tcoarse"] < 1440)]
        data_pd_evt["nearest_cluster"] = data_pd_evt["hit_id"].isin(hit_ids)
        data_pd_evt["nearest_cluster_str"] = data_pd_evt["nearest_cluster"].apply(
            lambda x: "Nearest cluster" if x else "Other hits")

        color_discrete_map = {"Other hits": 'cyan', "Nearest cluster": 'red'}
        data_pd_evt["time"] = (1569 - data_pd_evt["l1ts_min_tcoarse"])
        fig = px.scatter(data_pd_evt, "strip_y", "charge_SH", color="nearest_cluster_str",
                         color_discrete_map=color_discrete_map)
