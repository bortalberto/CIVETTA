...
#     cl_pd_2D=cl_pd_2D[0:100000]
    ...
    if tpc:
        cl_pd_2D.loc[~cl_pd_2D.cl_pos_x_tpc.notna(), "cl_pos_x_tpc"] = cl_pd_2D[~cl_pd_2D.cl_pos_x_tpc.notna()].cl_pos_x ## if there is no TPC position stored use cc position
...
def calculate_eff(run, data_folder, put, cpu_to_use, nsigma_put=5, nsigma_trackers=1, chi_sq_trackers=0, multi_tracks_suppresion=False, hit_efficiency=False, tpc=False):
    ...
    for put in tqdm( put_list, desc="Planar"):
        ...
        for view in ("x", "y"):
            # print (view)
            popt_list, pcov_list, res_list, R_list,chi_list, deg_list, nbins_list = r_fit.double_gaus_fit_root(tracks_pd, view, put)
            ...
            for pl in trackers_list:
                ...
                r_fit.plot_residuals(tracks_pd, view, popt_list, R_list, path_out_eff, put, mean_res, res_sigma, nsigma_trck, pl, chi_list, deg_list, chi_sq_trackers, nbins_list=nbins_list)
                # print ("plotted")
        ...
        view = "x"
        popt_list, pcov_list, res_list, R_list,chi_list, deg_list, nbins_list = r_fit.double_gaus_fit_root(tracks_pd_res, view)
        ...
        r_fit.plot_residuals(tracks_pd_res, view, popt_list, R_list, path_out_eff, put, put_mean_x, put_sigma_x, nsigma_put, put, chi_list, deg_list, nbins_list=nbins_list)
        print (R_list)
        print (R_list[put])
        if R_list[put] < 0.9:
        ...
        r_fit.plot_residuals(tracks_pd_res, view, popt_list, R_list, path_out_eff, put, put_mean_y, put_sigma_y, nsigma_put, put, chi_list, deg_list, nbins_list=nbins_list)
...
def calc_chi_sqrt(x, res_sigma_dict):
    """
    Calculate the N dimension chi squared sum, given the residual distributions sigmas
    """
    chi=0
